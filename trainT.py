...
def train_model(model, disc, n_epochs, num_timesteps, train_loader, val_loader, device):
    ...
    for i in range(0, n_epochs + 1):
        ...
        for _, (hr_data, lr_data) in enumerate(train_loader):
            ...
            # noisy_pred = q_sample(sr_slice, t)
            # disc_sr_noisy = disc(noisy_pred, t)

            original_pred = q_sample(sr_slice, torch.zeros_like(t))
            disc_sr_original = disc(original_pred, torch.zeros_like(t))

            # adversarial_loss_noisy = criterion_Disc(disc_sr_noisy, torch.ones_like(disc_sr_noisy).to(device))
            adversarial_loss_original = criterion_Disc(disc_sr_original, torch.ones_like(disc_sr_original).to(device))
            

            g_loss = gen_loss(sr_slice, hr_slice, adversarial_loss_original, criterion)
            g_loss.backward()
