...
class ResUnet(torch.nn.Module):
    def __init__(self, in_channel=1, filters=[64, 128, 256]):
        ...
        self.bridge = BasicBlock(filters[2], filters[2], (1,1,1), 1)
    ...
    def forward(self, x):
        ...
        x3_out = self.bridge(x3) #4->4

        x4 = self.upsample1(x3_out) #4 -> 8
        x4 = torch.cat([x4, x2], dim=1)
        x4 = self.up_residual_conv1(x4)

        x5 = self.upsample2(x4) #8->16
...
class Discriminator(nn.Module):
    def __init__(self, in_channels,features=[64, 128, 256], t_emb_dim = 128, act=nn.LeakyReLU(0.2)):
        super().__init__()
